...
final class SessionTests: XCTestCase {
  ...
  func testMessengerRoutesByRecipientAgentIDAndThread() throws {
    ...
    let source = try XCTUnwrap(sender.hand as? MessengerObject)
    ...
    XCTAssertTrue(output.contains("to agent #\(recipient.hash) in thread #project"))
  ...
  func testAttachProfileImmediatelyQueuesLatestUnread() throws {
    ...
    XCTAssertEqual(agent.pendingBroadcasts.first?.id, latest.id)
    XCTAssertNotEqual(agent.pendingBroadcasts.first?.id, first.id)
  }
  ...
  func testBroadcastRequiresPossession() throws {
    ...
    _ = try agent.backpack.container!.remove(at: messenger.coordinate!)
    try harness.place(messenger, at: Coordinate(x: 10, y: 10))
