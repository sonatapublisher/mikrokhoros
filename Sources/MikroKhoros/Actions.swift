  ...
  public var text: String {
    ...
    for event in events {
      ...
      case .broadcast(let broadcast):
        ...
        lines.append(
          "      body: \(scalar(broadcast.body, limit: AgentBroadcastEvent.maximumBodyCharacters))"
        )
...
public enum AgentActionKind: String, CaseIterable, Sendable {
  case move
  case container
  case pickup
  case drop
...
public struct AgentActionDefinition: Equatable, Sendable {
  public let kind: AgentActionKind
  public let syntax: String
  public let summary: String

  public init(kind: AgentActionKind, syntax: String, summary: String) {
    self.kind = kind
    self.syntax = syntax
...
public enum AgentActionCatalog {
  public static let all = AgentActionKind.allCases.map(definition(for:))

  public static func definition(for kind: AgentActionKind) -> AgentActionDefinition {
    switch kind {
    case .move:
      .init(
        kind: kind, syntax: "move <direction> [step]",
        summary: "Move through the current container space.")
    case .container:
      .init(
        kind: kind, syntax: "container in|out",
        summary: "Enter or leave a container at the current cell.")
    case .pickup:
      .init(kind: kind, syntax: "pickup", summary: "Pick up the object at the current cell.")
    case .drop:
      .init(
        kind: kind, syntax: "drop [<direction>|(<dx>,<dy>)]",
        summary: "Drop the held object at a relative cell.")
    case .backpack:
      .init(
        kind: kind, syntax: "backpack open|close", summary: "Enter or leave the carried backpack.")
    case .holding:
      .init(
        kind: kind, syntax: "holding select <1|2|3|4>",
        summary: "Select the primary carrying slot.")
    case .object:
      .init(
        kind: kind, syntax: "object (<function> (<argument>) ...)",
        summary: "Call a public function on the held or standing object.")
    case .inspect:
      .init(kind: kind, syntax: "inspect [here|held]", summary: "Inspect a public object surface.")
    }
  }

  public static var protocolSyntax: String { all.map(\.syntax).joined(separator: "\n") }

  public static func kind(for head: String) throws -> AgentActionKind {
    guard let kind = AgentActionKind(rawValue: head.lowercased()) else {
      throw MikroKhorosError.runtime(
        "command.unknown",
        "unknown action",
        suggestions: ["use \(AgentActionKind.allCases.map(\.rawValue).joined(separator: ", "))"]
      )
    }
    return kind
  }
...
public enum AgentProtocol {
  public static var instructions: String {
    ...
    Actions:
    \(AgentActionCatalog.protocolSyntax)
...
public final class AgentSession {
  private struct BroadcastKey: Hashable {
    let sourceID: String
    let eventID: String
  }
  ...
  public func request(broadcasts: [AgentBroadcastEvent]) throws -> AgentModelRequest {
    let uniqueBroadcasts = try Self.uniqueBroadcasts(broadcasts)
    guard uniqueBroadcasts.count <= harness.limits.maximumBroadcastEventsPerRequest else {
      throw MikroKhorosError.runtime(
    ...
    for event in uniqueBroadcasts {
      try PromptSafety.validateBroadcastEvent(event, limits: harness.limits)
      guard
        let source = harness.findObject(event.sourceID),
        let broadcastSource = source as? BroadcastSource,
        source.typeName == event.sourceType,
        harness.isCarried(source, by: agent),
        broadcastSource.unreadEvent(eventID: event.id) == event
      else {
    ...
    for (offset, action) in parsed.accepted.enumerated() {
      ...
      let queued = harness.performQueuedAction { () -> Result<String, RuntimeIssue> in
        let apply = { () -> Result<String, RuntimeIssue> in
          do {
            return .success(try self.interpreter.apply(action, for: self.agent))
          } catch let error as MikroKhorosError {
  ...
  func replayPersisted(_ commands: [String], operationID: String? = nil) throws {
    for (offset, command) in commands.enumerated() {
      let queued = harness.performQueuedAction { () -> Result<String, RuntimeIssue> in
        let apply = { () -> Result<String, RuntimeIssue> in
          do {
            return .success(try self.interpreter.applyPersisted(command, for: self.agent))
          } catch let error as MikroKhorosError {
            return .failure(error.issue)
          } catch {
            return .failure(
  ...
  public func handlePendingRequest(using client: any AICompleting) async throws -> AgentTurn? {
    try await handlePendingRequest(using: client, operationID: nil)
  }
  ...
  public func handlePendingRequest(
    ...
    do {
      ...
      let result = try await handlePendingRequestUnlocked(
        using: client,
        operationID: operationID
      )
  ...
  private func handlePendingRequestUnlocked(
    ...
    let drainedBroadcasts = harness.drainBroadcasts(for: agent)
    let broadcasts: [AgentBroadcastEvent]
    do {
      broadcasts = try Self.uniqueBroadcasts(drainedBroadcasts)
    } catch {
      agent.pendingBroadcasts.insert(contentsOf: drainedBroadcasts, at: 0)
      throw error
    }
    ...
    do {
      ...
      turn = run(response, operationID: operationID)
      // `run` reports action-language failures as a turn rather than throwing.
      // Treat a partially failed or empty turn as unaccepted so a source event
      // remains retryable; only an all-successful action turn may clear it.
      responseAccepted =
      ...
      restorePendingBroadcasts(broadcasts)
    ...
    // A source event is acknowledged only after output validation, action
    // acceptance, and model-history persistence all succeeded.  Failed
    // provider transport and rejected output leave the unread event retryable.
    if responseAccepted {
      acknowledgeDeliveredBroadcasts(broadcasts)
    }
    return turn
  }

  private func restorePendingBroadcasts(_ events: [AgentBroadcastEvent]) {
    guard !events.isEmpty else { return }
    let existingIDs = Set(
      agent.pendingBroadcasts.map {
        BroadcastKey(sourceID: $0.sourceID, eventID: $0.id)
      }
    )
    let restored = events.filter {
      !existingIDs.contains(BroadcastKey(sourceID: $0.sourceID, eventID: $0.id))
    }
    agent.pendingBroadcasts.insert(contentsOf: restored, at: 0)
  }

  private func acknowledgeDeliveredBroadcasts(_ events: [AgentBroadcastEvent]) {
    for event in events {
      guard let sourceObject = harness.findObject(event.sourceID),
        sourceObject.typeName == event.sourceType,
  ...
  private static func uniqueBroadcasts(
    _ events: [AgentBroadcastEvent]
  ) throws -> [AgentBroadcastEvent] {
    var seen: [BroadcastKey: AgentBroadcastEvent] = [:]
    var result: [AgentBroadcastEvent] = []
    result.reserveCapacity(events.count)
    for event in events {
      let key = BroadcastKey(sourceID: event.sourceID, eventID: event.id)
      if let prior = seen[key] {
        guard prior == event else {
          throw MikroKhorosError.runtime(
            "model.broadcast_duplicate_conflict",
            "duplicate broadcast identity has conflicting content"
          )
        }
        continue
      }
  ...
  private func modelInput(
    ...
    if broadcasts.isEmpty {
      ...
      for event in broadcasts {
        ...
        lines.append(
          "      body: \(scalar(event.body, limit: AgentBroadcastEvent.maximumBodyCharacters))"
        )
      }
    }
    lines.append("state:")
...
public final class ActionInterpreter {
  ...
  func applyPersisted(_ source: String, for agent: Agent) throws -> String {
    guard source.count <= harness.limits.maximumWorldBytes else {
      throw MikroKhorosError.persistence("persisted action exceeds the world limit")
    }
  ...
  private func applyValidated(_ action: String, for agent: Agent) throws -> String {
    ...
    let kind = try AgentActionCatalog.kind(for: String(split[0]))
    let remainder =
      split.count == 2
    ...
    case .holding:
      let arguments = try words(remainder)
      guard arguments.count == 2, arguments[0].lowercased() == "select",
        let raw = Int(arguments[1]), let number = try? HoldingNumber(raw)
      else {
        throw MikroKhorosError.command("usage: holding select 1|2|3|4")
      }
      try harness.selectHolding(number, for: agent)
      return "selected holding \(number.rawValue)"
